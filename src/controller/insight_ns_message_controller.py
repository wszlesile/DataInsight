from flask import Blueprint, jsonify

from controller.base_controller import BaseController
from service.insight_ns_message_service import InsightNsMessageService
from utils.response import Result


def create_insight_ns_message_controller(service: InsightNsMessageService) -> Blueprint:
    blueprint = Blueprint('insight_ns_message', __name__, url_prefix='/api/insight/message')
    controller = InsightNsMessageController(blueprint, service)

    blueprint.route('/', methods=['GET'])(controller.get_by_conversation)
    blueprint.route('/', methods=['POST'])(controller.create)
    blueprint.route('/', methods=['DELETE'])(controller.delete_by_conversation)
    return blueprint


class InsightNsMessageController(BaseController):
    """基础会话消息管理接口控制器。"""

    def __init__(self, blueprint: Blueprint, service: InsightNsMessageService):
        super().__init__(blueprint)
        self._service = service

    def get_by_conversation(self):
        data = self.get_json_data()
        insight_conversation_id = data.get('insight_conversation_id')
        if not insight_conversation_id:
            return self.error_response("缺少 insight_conversation_id 参数")

        messages = self._service.find_by_conversation_id(insight_conversation_id)
        return jsonify(Result.success(data=[self._to_dict(item) for item in messages]).to_dict())

    def create(self):
        data = self.get_json_data()
        username = data.get('username')
        insight_namespace_id = data.get('insight_namespace_id')
        insight_conversation_id = data.get('insight_conversation_id')
        message_type = data.get('type')
        content = data.get('content')
        insight_result = data.get('insight_result', '')

        if not all([username, insight_namespace_id, insight_conversation_id, message_type is not None, content]):
            return self.error_response("缺少必要参数")

        result = self._service.create_message(
            username,
            insight_namespace_id,
            insight_conversation_id,
            message_type,
            content,
            insight_result,
        )
        if result['success']:
            return self.success_response(result['data'], result['message'], 201)
        return self.error_response(result['message'], 400)

    def delete_by_conversation(self):
        data = self.get_json_data()
        insight_conversation_id = data.get('insight_conversation_id')
        if not insight_conversation_id:
            return self.error_response("缺少 insight_conversation_id 参数")

        result = self._service.delete_by_conversation_id(insight_conversation_id)
        return self.success_response(None, result['message'])

    def _to_dict(self, message) -> dict:
        return {
            "id": message.id,
            "username": message.username,
            "insight_namespace_id": message.insight_namespace_id,
            "insight_conversation_id": message.insight_conversation_id,
            "type": message.type,
            "content": message.content,
            "insight_result": message.insight_result,
            "created_at": message.created_at.isoformat() if message.created_at else None,
        }
