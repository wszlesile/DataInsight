...
数据库相关变量统一使用 `SUPOS_DATA_INSIGHT_*` 命名。
...
- 外置数据库和本地 SQLite 场景都统一使用 `SUPOS_DATA_INSIGHT_*` 这一组环境变量，与平台资源注入规范保持一致。
...
- 如果容器内使用 SQLite，`SUPOS_DATA_INSIGHT_SQLITE_PATH` 必须写成容器内路径，宿主机真实数据库文件通过 `-v` 挂载进去。
